...
class MutationEngine {
    ...
    constructor(config, prng) {
        this.config = config;
        this.prng = prng;
        this.strategyFns = new Map([
            [types_js_1.MutatorStrategy.BitFlip, this.bitFlip.bind(this)],
            [types_js_1.MutatorStrategy.ByteFlip, this.byteFlip.bind(this)],
            [types_js_1.MutatorStrategy.ArithmeticInc, this.arithInc.bind(this)],
            [types_js_1.MutatorStrategy.ArithmeticDec, this.arithDec.bind(this)],
            [types_js_1.MutatorStrategy.InterestingValues, this.interestingValues.bind(this)],
            [types_js_1.MutatorStrategy.DictionaryInsert, this.dictInsert.bind(this)],
            [types_js_1.MutatorStrategy.DictionaryOverwrite, this.dictOverwrite.bind(this)],
            [types_js_1.MutatorStrategy.Havoc, this.havoc.bind(this)],
            [types_js_1.MutatorStrategy.Splice, this.splice.bind(this)],
            [types_js_1.MutatorStrategy.Trim, this.trim.bind(this)],
            [types_js_1.MutatorStrategy.Extend, this.extend.bind(this)],
            [types_js_1.MutatorStrategy.StructureAware, this.structureAware.bind(this)],
            [types_js_1.MutatorStrategy.GrammarGuided, this.grammarGuided.bind(this)],
            [types_js_1.MutatorStrategy.TokenLevel, this.tokenLevel.bind(this)],
        ]);
    }
    /** Mutate an input using configured strategies. */
    mutate(input, maxSize, corpus) {
    ...
    interestingValues(input, _maxSize) {
        if (input.length === 0)
            return input;
        const buf = cloneBytes(input);
        const width = this.prng.pick([1, 2, 4].filter(w => w <= buf.length));
    ...
    dictOverwrite(input, _maxSize) {
        if (this.dictionary.length === 0 || input.length === 0)
            return cloneBytes(input);
        const entry = this.prng.pick(this.dictionary);
    ...
    splice(input, maxSize, corpus) {
        if (corpus.length < 2)
            return cloneBytes(input);
        const other = this.prng.pick(corpus);
        if (other.data.length === 0 || input.length === 0)
            return cloneBytes(input);
...
function splitByDelimiters(data, delimiters) {
    const chunks = [];
    let start = 0;
