...
class CoverageTracker {
    ...
    snapshot() {
        ...
        if (this._globalHitCount === 0) {
            return {
                ...
                percentage: 0,
                metric: this.config.metrics[0] ?? types_js_1.CoverageMetric.Edge,
            };
        }
        ...
        const totalHit = (0, hash_js_1.countNonZero)(edgeBitmap);
        return {
            edgeBitmap,
    ...
    findNewEdges(snap) {
        ...
        for (let i = 0; i < snap.edgeBitmap.length; i++) {
            ...
            if (traceVal === 0)
                continue;
            const classified = (0, hash_js_1.classifyCount)(traceVal);
    ...
    updateStability(isDeterministic) {
