...
exports.handleCorpus = handleCorpus;
...
function buildConfigFromFlags(flags) {
    ...
    if (flags.has('target')) {
        config.harness = {
            mode: flags.get('harness') ?? types_js_1.HarnessMode.ForkServer,
    ...
    if (flags.has('network-target')) {
        config.harness = {
            mode: types_js_1.HarnessMode.Network,
...
function handleVersion() {
    return {
        exitCode: 0,
        output: `LombokFuzzer v${index_js_1.VERSION}`,
    };
}
...
async function cli(argv) {
    ...
    switch (parsed.command) {
        ...
        case CliCommand.Run: {
            ...
            return {
                exitCode: 0,
                output: `LombokFuzzer v${index_js_1.VERSION} — ready to run campaign "${_mergedConfig.name ?? 'unnamed'}"`,
            };
        }
...
// ─── Help Text ──────────────────────────────────────────────────────────────
const HELP_MAIN = `
LombokFuzzer v${index_js_1.VERSION} — Universal fuzzing framework
...
`.trim();
const HELP_RUN = `
LombokFuzzer v${index_js_1.VERSION} — run command
