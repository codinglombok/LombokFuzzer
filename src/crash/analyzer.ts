...
export class CrashAnalyzer {
  ...
  private readonly _crashes = new Map<string, CrashInfo>();
  private readonly _stackHashes = new Set<string>();
  ...
  analyze(result: ExecutionResult): CrashInfo | null {
    ...
    if (isDuplicate) {
      ...
      if (existing) existing.reproCount++;
    } else {
  ...
  private classifyCrash(result: ExecutionResult, _frames: StackFrame[]): CrashCategory {
    ...
    if (sanitizer.includes('heap-buffer-overflow')) return CrashCategory.HeapOverflow;
    if (sanitizer.includes('stack-buffer-overflow')) return CrashCategory.StackOverflow;
    if (sanitizer.includes('heap-use-after-free')) return CrashCategory.UseAfterFree;
    if (sanitizer.includes('null') && sanitizer.includes('dereference')) return CrashCategory.NullDeref;
    if (sanitizer.includes('integer-overflow')) return CrashCategory.IntegerOverflow;
    if (sanitizer.includes('data race')) return CrashCategory.RaceCondition;
    if (sanitizer.includes('out-of-bounds') || sanitizer.includes('out of bounds')) return CrashCategory.BufferOverflow;

    // ── Message-based classification (match first line only) ──
    if (message.includes('out of memory') || message.includes('heap out of memory')) return CrashCategory.OOM;
    if (message.includes('integer overflow')) return CrashCategory.IntegerOverflow;
    if (message.includes('division by zero') || message.includes('divide by zero')) return CrashCategory.DivisionByZero;
    if (message.includes('format string')) return CrashCategory.FormatString;
    if (message.includes('assertion')) return CrashCategory.Assertion;
    if (message.includes('data race') || message.includes('race condition')) return CrashCategory.RaceCondition;

    // Signal-based (native harnesses)
    if (result.signal === 11) return CrashCategory.BufferOverflow; // SIGSEGV
    if (result.signal === 6) return CrashCategory.Assertion;       // SIGABRT
    if (result.signal === 8) return CrashCategory.DivisionByZero;  // SIGFPE

    // ── JavaScript error types (match message line) ──
    if (message.includes('rangeerror')) {
      if (message.includes('call stack') || message.includes('recursion')) return CrashCategory.StackOverflow;
    ...
    if (message.includes('cross-site scripting') || message.includes('xss')) return CrashCategory.XSS;
    if (message.includes('ssrf') || message.includes('server-side request forgery')) return CrashCategory.SSRF;
    if (message.includes('deserializ')) return CrashCategory.Deserialization;

    return CrashCategory.Unknown;
  }

  /** Assess severity based on crash category. */
  private assessSeverity(category: CrashCategory, _result: ExecutionResult): Severity {
    ...
    if (critical.has(category)) return Severity.Critical;
    if (high.has(category)) return Severity.High;
    if (medium.has(category)) return Severity.Medium;
  ...
  private signalName(signal: number): string {
