...
export class HttpFuzzer implements ProtocolFuzzerModule {
  readonly name = 'HTTP';

  private readonly methods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD',
  ...
  private genPath(prng: PRNG): string {
  ...
  private genHost(prng: PRNG): string {
    const len = 3 + prng.nextRange(10);
    let host = '';
    for (let i = 0; i < len; i++) host += String.fromCharCode(97 + prng.nextRange(26));
    return `${host}.com`;
  }

  private genHeaderValue(name: string, prng: PRNG): string {
    ...
    if (common[name]) return prng.pick(common[name]!);
    const len = 1 + prng.nextRange(50);
    let val = '';
    for (let i = 0; i < len; i++) val += String.fromCharCode(32 + prng.nextRange(95));
    return val;
  }

  private genBody(prng: PRNG): string {
    const bodyType = prng.nextRange(3);
    if (bodyType === 0) return `{"key":"${prng.pick(this.attackPayloads)}"}`;
    if (bodyType === 1) return `name=${prng.pick(this.attackPayloads)}&value=test`;
    const len = prng.nextRange(1024);
    let body = '';
    for (let i = 0; i < len; i++) body += String.fromCharCode(32 + prng.nextRange(95));
    return body;
  }
}
...
export class DnsFuzzer implements ProtocolFuzzerModule {
  readonly name = 'DNS';
  ...
  generate(prng: PRNG): ProtocolMessage {
    ...
    for (let i = 0; i < 6; i++) buf[offset++] = 0x00;
  ...
  mutate(msg: ProtocolMessage, prng: PRNG): ProtocolMessage {
    ...
    switch (mutation) {
      case 0: // Corrupt label length (pointer loop)
        if (buf.length > 12) buf[12] = 0xC0; // compression pointer
        break;
      case 1: // Oversized label
        if (buf.length > 12) buf[12] = 64; // max label is 63
        break;
...
export class WebSocketFuzzer implements ProtocolFuzzerModule {
  ...
  generate(prng: PRNG): ProtocolMessage {
    ...
    frame.set(header, 0);
    if (mask.length > 0) frame.set(mask, header.length);
  ...
  mutate(msg: ProtocolMessage, prng: PRNG): ProtocolMessage {
    ...
    switch (mutation) {
      case 0: // Invalid opcode
        if (buf.length > 0) buf[0] = (buf[0]! & 0xF0) | prng.nextRange(16);
        break;
      case 1: // Wrong length
        if (buf.length > 1) buf[1] = (buf[1]! & 0x80) | prng.nextRange(128);
        break;
      case 2: // RSV bits set (reserved, should be 0)
        if (buf.length > 0) buf[0] = buf[0]! | 0x70;
        break;
      case 3: // Fragment control frame
        if (buf.length > 0) buf[0] = buf[0]! & ~0x80; // clear FIN on control frame
        break;
...
export class ProtocolFuzzerRegistry {
  private readonly modules = new Map<string, ProtocolFuzzerModule>();
