...
export class ApiFuzzer {
      ...
      for (const [method, operation] of Object.entries(methods)) {
        ...
        const endpoint: ApiEndpoint = {
          ...
          parameters: params.map(p => ({
            ...
            schema: (p['schema'] as ApiSchema) ?? { type: 'string' },
          })),
          requestBody: reqBody
  ...
  private generateString(schema: ApiSchema): string {
    if (schema.enum) return String(this.prng.pick(schema.enum as string[]));
    ...
    let s = '';
    for (let i = 0; i < len; i++) s += String.fromCharCode(97 + this.prng.nextRange(26));
    return s;
  }

  private generateNumber(schema: ApiSchema): number {
  ...
  private generateArray(schema: ApiSchema): unknown[] {
  ...
  private generateObjectFromSchema(schema: ApiSchema): Record<string, unknown> {
  ...
  private attackPayload(type: string): unknown {
    const strPayloads = [
    ...
    if (type === 'string') return this.prng.pick(strPayloads);
    if (type === 'integer' || type === 'number') return this.prng.pick(numPayloads);
    return null;
  }

  private applyAuth(headers: Record<string, string>, query: Record<string, string>): void {
    if (!this.auth) return;
  ...
  private extractSchema(reqBody: Record<string, unknown>): ApiSchema {
    const content = reqBody['content'] as Record<string, Record<string, unknown>> | undefined;
    if (content?.['application/json']?.['schema']) {
      return content['application/json']['schema'] as ApiSchema;
    }
    return { type: 'object' };
  }

  private hasInfoDisclosure(body: string): boolean {
    const patterns = [
  ...
  private hasReflectedPayload(request: ApiRequest, body: string): boolean {
    const payloads = [
