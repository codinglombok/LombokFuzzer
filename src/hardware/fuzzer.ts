...
export class HardwareFuzzer {
  private readonly prng: PRNG;
  private readonly findings: HWFinding[] = [];
  ...
  generateRegisterFuzz(registers: RegisterMap): HWFuzzCommand[] {
    ...
    for (const [name, reg] of Object.entries(registers)) {
      if (reg.access === 'r') continue; // Skip read-only
  ...
  private fuzzUART(_target: HWFuzzTarget): HWFuzzCommand {
  ...
  private fuzzSPI(_target: HWFuzzTarget): HWFuzzCommand {
  ...
  private fuzzI2C(_target: HWFuzzTarget): HWFuzzCommand {
  ...
  private fuzzDebugPort(target: HWFuzzTarget): HWFuzzCommand {
  ...
  private generateMalformedUART(): Uint8Array {
...
function numberToBytes(val: number, byteLen: number): Uint8Array {
